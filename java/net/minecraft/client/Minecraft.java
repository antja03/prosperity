...
//        }
    ...
    private void runGameLoop() throws IOException
    {
        ...
        StartFrameEvent.INSTANCE.dispatch();
    ...
    {
        GameTickEvent.INSTANCE.makePre().dispatch();
        ...
        GameTickEvent.INSTANCE.makePost().dispatch();
    }
    ...
    public void runTickMouse() throws IOException {
        while (Mouse.next())
        {
            ...
            if (currentScreen == null && i == 0 || i == 1) {
                if (Mouse.getEventButtonState()) {
                    MouseDownEvent event = MouseDownEvent.INSTANCE;
                    event.setMouseButton(i);
                    event.dispatch();

                    if (event.isCancelled())
                        return;
                } else {
                    MouseUpEvent event = MouseUpEvent.INSTANCE;
                    event.setMouseButton(i);
                    event.dispatch();
    ...
    public void runTickKeyboard() throws IOException {
        while (Keyboard.next())
        {
            ...
            if (currentScreen == null && k <= 256 && !Keyboard.isRepeatEvent()) {
                if (Keyboard.isKeyDown(k)) {
                    KeyDownEvent event = KeyDownEvent.INSTANCE;
                    event.setKeyCode(k);
                    event.dispatch();
                    ...
                    KeyUpEvent event = KeyUpEvent.INSTANCE;
                    event.setKeyCode(k);
                    event.dispatch();
